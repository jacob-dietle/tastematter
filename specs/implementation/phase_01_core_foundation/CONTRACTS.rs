//! Type Contracts for Phase 1: Core Foundation
//!
...
//!
//! Copy these types directly into src/types.rs when implementing.
...
/// The QueryEngine trait defines the public API for Phase 1
/// Phase 2 will call these methods from Tauri commands
pub trait QueryEngineApi {
    /// Query files with flexible filters
    fn query_flex(
        &self,
        input: QueryFlexInput,
    ) -> impl std::future::Future<Output = Result<QueryResult, CoreError>> + Send;

    /// Query timeline data for visualization
    fn query_timeline(
        &self,
        input: QueryTimelineInput,
    ) -> impl std::future::Future<Output = Result<TimelineData, CoreError>> + Send;

    /// Query session-grouped data
    fn query_sessions(
        &self,
        input: QuerySessionsInput,
    ) -> impl std::future::Future<Output = Result<SessionQueryResult, CoreError>> + Send;

    /// Query chain metadata
    fn query_chains(
        &self,
        input: QueryChainsInput,
    ) -> impl std::future::Future<Output = Result<ChainQueryResult, CoreError>> + Send;
}
