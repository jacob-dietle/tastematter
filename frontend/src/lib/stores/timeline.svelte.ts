...
export function createTimelineStore(ctx?: ContextStore) {
  ...
  // Get chain filter from context if available
  function getChainFilter(): string | undefined {
    return ctx?.selectedChain ?? undefined;
  }
  ...
  async function fetch() {
    ...
    error = null;
    try {
      const result = await queryTimeline({ time: getTimeRange(), chain: getChainFilter(), limit: QUERY_LIMITS.timeline });
