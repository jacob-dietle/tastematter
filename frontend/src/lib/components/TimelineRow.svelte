<script lang="ts">
  ...
  interface Props {
    ...
    onLeave?: () => void;
    onSelect?: (file: string, date: string) => void;
  }

  let { filePath, dates, buckets, maxCount, onHover, onLeave, onSelect }: Props = $props();
  ...
  function handleActivate(date: string) {
    onSelect?.(filePath, date);
  }

  function handleKeydown(event: KeyboardEvent, date: string) {
  ...
  <div class="cells">
    {#each dates as date}
      <div
        ...
        role="button"
        tabindex={buckets[date] > 0 ? 0 : -1}
        aria-label="{buckets[date] ?? 0} accesses on {date}"
        onmouseenter={() => handleMouseEnter(date)}
        onmouseleave={handleMouseLeave}
        onclick={() => handleActivate(date)}
        onkeydown={(e) => handleKeydown(e, date)}
        onfocus={() => handleMouseEnter(date)}
...
<style>
  ...
  .heat-cell:focus:not(:focus-visible) {
    outline: none;
  }
</style>
