...
<!-- Sessions view - uses backend filtering via chain parameter -->
<script lang="ts">
  ...
  // Fetch sessions with backend chain filtering
  ...
  // Fetch on mount and when timeRange or selectedChain changes
  $effect(() => {
    const _time = ctx.timeRange; // dependency
    const _chain = ctx.selectedChain; // dependency
    fetchSessions();
  });

  // Client-side filtering by selectedChain (in addition to backend filtering)
  function getFilteredSessions(): SessionData[] {
    if (!ctx.selectedChain) {
      return sessions;
    }
    return sessions.filter(session => session.chain_id === ctx.selectedChain);
  }
