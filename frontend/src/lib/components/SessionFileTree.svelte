    ...
    >
      {#if node.type === 'directory'}
        <button
          ...
          aria-label="{isDirExpanded(node.path) ? 'Collapse' : 'Expand'} {node.name} directory"
          aria-expanded={isDirExpanded(node.path)}
        >
      ...
      {:else}
        ...
        <button
          ...
          aria-label="Open {node.name}, {node.access_count} accesses"
        >
          {node.name}
        </button>
