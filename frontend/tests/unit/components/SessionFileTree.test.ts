...
describe('SessionFileTree', () => {
  ...
  test('shows directories with expand/collapse arrows', () => {
    ...
    // Should show expand/collapse buttons for directories (match aria-label pattern)
    const toggleButtons = screen.getAllByRole('button', { name: /Expand|Collapse/ });
  ...
  test('toggles directory expansion on click', async () => {
    ...
    // Find a directory toggle button by aria-label
    const toggleButtons = screen.getAllByRole('button', { name: /Expand|Collapse/ });
